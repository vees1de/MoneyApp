...
func sourceApplications() SourceDef {
	return SourceDef{
		...
		Columns: []ColumnDef{
			...
			{Key: "max_price", Label: "Max price", Type: "currency", Default: true,
				SQLExpr: "CASE WHEN ci.max_participants IS NOT NULL AND ci.max_participants > 0 AND ci.price IS NOT NULL THEN ci.max_participants * ci.price ELSE NULL END", SQLAlias: "max_price"},
			{Key: "current_price", Label: "Current price", Type: "currency", Default: true,
				SQLExpr: "CASE WHEN ci.price IS NOT NULL THEN (SELECT count(*) FROM course_applications ca2 WHERE ca2.intake_id = ca.intake_id AND ca2.status IN ('approved','enrolled')) * ci.price ELSE NULL END", SQLAlias: "current_price"},
	...
	return SourceDef{
		...
		Columns: []ColumnDef{
			...
			{Key: "max_price", Label: "Max price", Type: "currency", Default: true,
				SQLExpr: "CASE WHEN ci.max_participants IS NOT NULL AND ci.max_participants > 0 AND ci.price IS NOT NULL THEN ci.max_participants * ci.price ELSE NULL END", SQLAlias: "max_price"},
			{Key: "current_price", Label: "Current price", Type: "currency", Default: true,
				SQLExpr: "CASE WHEN ci.price IS NOT NULL THEN (SELECT count(*) FROM course_applications ca WHERE ca.intake_id = ci.id AND ca.status IN ('approved','enrolled')) * ci.price ELSE NULL END", SQLAlias: "current_price"},
