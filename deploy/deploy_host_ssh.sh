...
#   - DEPLOY_REPO_URL env var points to the git repository
#   - DEPLOY_GIT_REF env var points to the branch/tag to deploy
# Steps:
...
Also requires DEPLOY_REPO_URL and DEPLOY_GIT_REF in the environment.
EOF
}
...
if [[ -z "${DEPLOY_REPO_URL}" ]]; then
  echo "missing DEPLOY_REPO_URL. It should point to the repository to deploy." >&2
  exit 1
fi

if [[ -z "${DEPLOY_GIT_REF}" ]]; then
  echo "missing DEPLOY_GIT_REF. It should point to the branch or tag to deploy." >&2
  exit 1
fi
